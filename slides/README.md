...
## 管理内容／本社と拠点役割分担（レビュー修正版）

`build_slides_mgmt.py`。既存2枚（P11／P12）をレビュー指摘に沿って作り直したもの。
マスターはダイセル標準テンプレート、書体はMeiryo。本文配色は元スライドに合わせた
濃紺パネル `1F3864` ＋クリーム帯 `FDF6E0` ＋淡青表ヘッダ `D9E2F3`。

主な修正点：

- 見出しを結論文に変更（「管理内容」→「生準の日常管理は、進捗と課題の2つに絞る」）
- P11の「責任部門を指示する」を「責任の明確化」に変更し、決め方は論点として提起
  （P12の協業モデルとの矛盾を解消）
- P12冒頭に依頼事項（承認依頼）を明示。「進めたい」を排した
- マトリックスに用語の定義（主体／支援／共有／情報提供）を追加
- 「考え方」列を「関与内容」に変更し、主体欄の言い換えになっていた記述を実質化
- ①②の項目軸を「管理単位／記録項目／更新契機・更新者」に統一
- 未確定箇所は［要確認：…］として明示。P12の未決3点は右上の要確認タグに集約
